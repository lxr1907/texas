package com.lxrtalk.texas.texas.threeCard;
...
import com.lxrtalk.texas.entity.PlayerOpt;
import com.lxrtalk.texas.entity.PlayerPrivate;
...
import com.lxrtalk.texas.texas.BetPool;
...
public class ThreeCardTexasRoom extends TexasRoom {
	private static Logger logger = LogManager.getLogger(ThreeCardTexasRoom.class);
	/**
	 * 初始锅底，回收的筹码
	 */
	@Expose
	...
	public void startGame() {
		...
		if (getWaitPlayers().size() < getConfig().getMinPlayers()) {
			return;
		}
		if (this.getGamestate().compareAndSet(0, 1)) {
			...
			for (Player p : getWaitPlayers()) {
				...
				p.setBetTimes(0);
				// 重新补筹码
				if (p.getBodyChips() != getConfig().getMaxChips()) {
	...
	public boolean betchipIn(Player player, int chip, boolean playerOpt) {
		...
		if (playerOpt) {
			...
			// 看牌下注基数和暗牌下注基数不同
			if (lookCard) {
				betBase = getConfig().getSmallBet();
			} else {
				betBase = this.getSmallBlindBet();
			}
			// 不能小于之前下注倍数
			if (chip / betBase < thisTexasRoom.getRoundMaxBet()) {
				logger.info("< getRoundMaxBet() chip:" + chip + "oldBetThisRound:" + oldBetThisRound + " max:"
						+ thisTexasRoom.getRoundMaxBet());
				return false;
			}
			// 下注必须是最小基数的整数倍
			if (chip % betBase != 0) {
			...
			// 设置本轮最大加注是基础加注的倍数
			if (chip / betBase > thisTexasRoom.getRoundMaxBet()) {
				thisTexasRoom.setRoundMaxBet(chip / betBase);
			}
		}
		if (playerOpt) {
	...
	public void endGame() {
		...
		if (this.getGamestate().compareAndSet(1, 2)) {
			...
			try {
				// 等待一段时间
				Thread.sleep(getConfig().getRestBetweenGame());
				// 判断是否可以开始下一局
				checkStart(1000);
