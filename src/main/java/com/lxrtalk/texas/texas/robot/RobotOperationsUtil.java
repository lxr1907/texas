...
import com.lxrtalk.texas.utils.RandomNumUtil;
...
public class RobotOperationsUtil {
    ...
    private static final int ALL_ROBOT_COUNT = 2000;

    static {
        // 初始化2000个机器人id
        for (int i = 0; i < ALL_ROBOT_COUNT; i++) {
     ...
     *
     */
    public synchronized static void robotLogin(RobotWsClient robotClient) {
        RobotPlayer robotPlayer = new RobotPlayer();
        robotPlayer.setC(1);
        Integer id = robotAccountQueue.poll();
        if (id == null) {
            logger.error("机器人账号已用完");
            return;
        }
    ...
    public static void onLogin(RobotWsClient robotClient, RetMsg retMsg) {
        ...
        if (retMsg.getState() == 1) {
            ...
            logger.error("机器人登陆失败，robotId：" + robotClient.player.getRobotAccountId());
        }
    }
